...
//         console.log(`Fetched ${data.length} persons in ${endTime - startTime}ms`);
//         res.status(200).json(data);
//     } catch (error) {
//         console.log(error);
//         res.status(500).json({ error: 'Internal server error' });
...
// // GET endpoint to fetch persons by work type
// router.get('/:workType', async (req, res) => {
//     try {
//         const workType = req.params.workType;
...
//             const response = await Person.find({ work: workType });
//             const endTime = Date.now();
//             console.log(`Fetched ${response.length} persons of type ${workType} in ${endTime - startTime}ms`);
//             res.status(200).json(response);
//         } else {
//             res.status(404).json({ error: 'Invalid work type' });
//         }
...
router.get('/:workType', async (req, res) => {
    try {
        const workType = req.params.workType;
        if (['chef', 'manager', 'waiter'].includes(workType)) {
            console.log(`Fetching persons with work type: ${workType}`);
            const response = await Person.find({ work: workType });
